...
def approval_program():
    ...
    def verify_deposit(tx_index: Int, receiver: Addr):
        return Assert(And(
            ...
            Gtxn[tx_index].xfer_asset() == Int(1200000),
    ...
    program = Cond(
        ...
        [Txn.application_args[0] == Bytes(
            "add_deposit_accounts"), add_deposit_accounts],
...
if __name__ == "__main__":
    dao_application_approval_path = (
        Path("..") / ".." / "ui" / "src" / "contracts" /
        "dao_application_approval.teal"
    )
    dao_application_clear_path = (
        Path("..") / ".." / "ui" / "src" / "contracts" /
        "dao_application_clear.teal"
    )

    with dao_application_approval_path.open("w") as f:
        compiled = compileTeal(
            approval_program(), mode=Mode.Application, version=5)
        f.write(compiled)

    with dao_application_clear_path.open("w") as f:
